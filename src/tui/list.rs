...
impl ListApp {
    pub fn new(config: Config) -> Self {
        let fossils: Vec<(String, TrackedFile)> = config
            .fossils
            .iter()
        ...
        if let Ok(entries) = fs::read_dir(dir) {
            for entry in entries.flatten() {
                ...
                if path.is_file() {
                    ...
                    let is_tracked = config.fossils.values().any(|f| f.original_path == path_str);
    ...
    fn draw_sidebar<B: Backend>(&self, f: &mut Frame<B>, area: Rect) {
        ...
        // Layers panel
        let layers_items: Vec<ListItem> = self
            .layers
            .iter()
            .map(|layer| {
                ...
                ListItem::new(format!("Layer {}{}", layer, current_marker)).style(
                    if *layer == self.config.current_layer {
                        Style::default()
                            .fg(Color::Yellow)
    ...
    fn draw_main_table<B: Backend>(&mut self, f: &mut Frame<B>, area: Rect) {
        ...
        let rows = self
            ...
            .map(|(i, (hash, tracked_file))| {
                ...
                let tags: Vec<String> = tracked_file
                    .layer_versions
                    .iter()
                    .filter_map(|lv| {
                        if lv.tag.is_empty() {
                ...
                let cells = vec![
                    ...
                    tui::widgets::Cell::from(tags_str).style(Style::default().fg(Color::Cyan)),
